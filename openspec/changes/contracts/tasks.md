...
- [ ] 1.1 Create `packages/contracts/package.json` — `@beim/contracts`, private, exports `src/index.ts`, devDeps zod + vitest
- [ ] 1.2 Create `packages/contracts/tsconfig.json` — extend `@beim/tsconfig/base.json`, outDir `dist`, rootDir `src`
- [ ] 1.3 Create `packages/contracts/vitest.config.ts` — include `src`, coverage `@vitest/coverage-v8`
- [ ] 1.4 Create `src/enums.ts` — `UserRole`, `Currency`, `OrderStatus`, `PaymentStatus`, `StockMovementType` as `z.enum([...as const])` with value + type exports
- [ ] 1.5 Create `src/index.ts` barrel — re-export enums only initially
- [ ] 1.6 Run `pnpm install` — resolve zod + vitest workspace deps

## Phase 2: Commerce Schemas

- [ ] 2.1 Create `src/user.ts` — `userSchema` + `User`; required: name, passwordHash, role; optional via `.exactOptional()`
- [ ] 2.2 Create `src/product.ts` — `productSchema` + `Product`; `price: z.number()`, `currency: Currency`, `compatibleModels: z.array(z.string())`
- [ ] 2.3 Create `src/category.ts` — `categorySchema` + `Category`; required: name, code, description
- [ ] 2.4 Create `src/order.ts` — `orderSchema` + `Order`; required: customer, total, currency, status, paymentStatus
- [ ] 2.5 Create `src/order-item.ts` — `orderItemSchema` + `OrderItem`; `quantity: z.number().int().positive()`
- [ ] 2.6 Update `src/index.ts` — explicit named exports for all 5 commerce schemas + types
