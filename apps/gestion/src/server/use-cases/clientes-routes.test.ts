...
describe("/api/gestion/clientes routes (remote-only)", () => {
  beforeAll(async () => {
    ...
    for (const cookie of [adminCookieWithBearer, sellerCookie]) {
    ...
    it("ignores a forged client-side role and authorizes from the session", async () => {
      vi.stubGlobal(
        ...
        vi.fn().mockResolvedValue(
          remoteOk([{ id: "c_1", ownerId: "u-remote", version: 1, displayName: "Cliente", active: true }])
        )
      );
  ...
  describe("PATCH/PUT/DELETE /api/gestion/clientes/[id]", () => {
    ...
    it("returns 503 next-implementation for PATCH and never calls fetch", async () => {
      const fetchSpy = vi.fn().mockRejectedValue(new Error("network must not be called"));
      vi.stubGlobal("fetch", fetchSpy);
    ...
    it("returns 503 next-implementation for DELETE and never calls fetch", async () => {
      const fetchSpy = vi.fn().mockRejectedValue(new Error("network must not be called"));
      vi.stubGlobal("fetch", fetchSpy);
