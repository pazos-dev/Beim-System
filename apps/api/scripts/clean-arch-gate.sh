...
npx vitest run src/contracts/contract-freeze.test.ts src/app.test.ts src/docs/openapi.test.ts src/errors/errors.test.ts src/domain/shared/types.test.ts src/domain/shared/ports.test.ts src/domain/shared/error-catalog.test.ts src/domain/user/user.test.ts src/domain/product/product.test.ts src/domain/product/stock-lot.test.ts src/domain/service/service.test.ts src/domain/venta/venta.test.ts src/domain/pago/pago.test.ts src/domain/receipt/receipt.test.ts src/domain/cash-session/cash-session.test.ts src/domain/stock-journal/stock-journal.test.ts src/domain/settings/settings.test.ts src/application/auth/handlers.test.ts src/application/catalog/product-handlers.test.ts src/application/catalog/service-handlers.test.ts src/application/purchases/purchases.test.ts src/composition-root.test.ts src/infrastructure/persistence/error-map.test.ts src/infrastructure/persistence/pg-user-session.test.ts src/infrastructure/persistence/pg-user-session.pg.test.ts src/infrastructure/persistence/pg-product-lots.test.ts src/infrastructure/persistence/pg-product-lots.pg.test.ts src/infrastructure/persistence/pg-service.test.ts src/infrastructure/persistence/pg-service.pg.test.ts src/infrastructure/persistence/pg-cash.test.ts src/infrastructure/persistence/pg-cash.pg.test.ts src/infrastructure/persistence/pg-pago.test.ts src/infrastructure/persistence/pg-pago.pg.test.ts src/infrastructure/persistence/pg-idempotency.test.ts src/infrastructure/persistence/pg-idempotency.pg.test.ts src/infrastructure/persistence/pg-venta.test.ts src/infrastructure/persistence/pg-venta.pg.test.ts src/infrastructure/persistence/pg-receipt.test.ts src/infrastructure/persistence/pg-receipt.pg.test.ts src/infrastructure/persistence/pg-settings.test.ts src/infrastructure/persistence/pg-settings.pg.test.ts src/infrastructure/storage/s3-storage.adapter.test.ts src/infrastructure/payments/mp-preference-gateway.test.ts src/infrastructure/payments/mp-webhook-verifier.test.ts src/middleware/no-shims.test.ts src/infrastructure/errors/toAppError.test.ts src/infrastructure/db/pool.test.ts src/infrastructure/clock/system-clock.test.ts src/infrastructure/uuid/system-uuid.test.ts src/application/shared/unit-of-work.test.ts src/application/sales-batch/sales-batch.test.ts src/application/receipts/receipts.test.ts src/application/payments/payments.test.ts src/application/cash/cash.test.ts src/application/orders/orders.test.ts src/application/creates/creates.test.ts src/application/uploads/uploads.test.ts src/application/ticket/ticket.test.ts src/application/queries/queries.test.ts src/application/queries/queries-passthrough.test.ts src/config/db.test.ts src/interface/http/error-matrix.test.ts src/interface/http/edge/edge-parity.test.ts src/interface/http/user/router.test.ts src/middleware/auth.test.ts src/middleware/cors.test.ts src/middleware/validate.test.ts src/middleware/rate-limit.test.ts src/middleware/rate-limit-store.test.ts src/interface/http/product/router.test.ts src/interface/http/service/router.test.ts src/interface/http/venta/router.test.ts src/interface/http/receipt/router.test.ts src/interface/http/caja/router.test.ts src/interface/http/pago/router.test.ts src/interface/http/upload/router.test.ts src/interface/http/clients/router.test.ts src/interface/http/categories/router.test.ts src/interface/http/purchases/router.test.ts src/cutover/user-adapters.test.ts src/cutover/invoice-pdf-adapter.test.ts src/cutover/mounting.test.ts src/application/reports/reports.test.ts src/application/audit/audit.test.ts src/application/finance/finance.test.ts src/interface/http/reports/router.test.ts src/interface/http/audit/router.test.ts src/interface/http/finance/router.test.ts src/application/catalog/catalog-reads.test.ts src/interface/http/catalog/router.test.ts src/application/orders/orders-reads.test.ts src/interface/http/orders-read/router.test.ts
...
echo "== cutover import scan (wiring delegates only: no pg/pool/drivers/zod) =="
if grep -rEn "from ['\"]pg['\"]|new Pool\(|from ['\"][^'\"]*(aws-sdk|mercadopago)|from ['\"]zod['\"]" src/cutover/ --include="*.ts" | grep -v "\.test\.ts"; then
  echo "forbidden import in src/cutover production code (adapters delegate to ports only)" >&2
  exit 1
fi
